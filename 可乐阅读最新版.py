...
由于回调服务器失效，没有key的可以临时填xxx.

2.打开活动入口，抓包的任意接口饼干参数
3.青龙配置文件，添加本脚本环境变量
填写变量参数时为方便填写可以随意换行
单账户：导出klydconfig="[{'名称'：'备注名'，'cookie'：'PHPSESSID=xxxx；udtauth3=a267Rxxxxx'，'key'：'xxxxxxx'，'uids'：'xxxxxxx'}]"
多账户：导出klydconfig="[
...
参数说明：
姓名：备注名随意填写
饼干：打开活动入口，抓包的任意接口页眉中的饼干参数
钥匙：每个账号的推送标准，每个账号全阅读只需要一个key，多个账号需要多个钥匙，钥匙永不过期。
为了防止恶意调用key接口，限制每个ip每天只能获取一个key.手机开飞行模式10s左右可以变更ip重新获取key
通过浏览器打开链接获取: http://175.24.153.42:8882/getkey
UID:wxpusher的参数，当一个微信关注了一个wxpusher的推送应用后，会在推送管理后台(https://wxpusher.zjiecode.com/admin/main)的'用户管理-->用户列表'中显示
...
方式二：配置文件添加
出口push_config="{'printf'：0，'threadingf'：1，'threadingt'：3，'AppToken'：'xxxx'}"
参数说明：
printf:0是不打印调试日志，1是打印调试日志
螺纹f：并行运行账号参数1并行执行，0顺序执行，并行执行优点，能够并行跑所以账号，加快完成时间，缺点日志打印混乱。
螺纹t：并行运行时每个账号的间隔时间默认3s
AppToken这个是填wxpusher的AppToken，找不到自己百度

5.本地电脑python运行
在本脚本最下方代码如果__name__=='__main__'：下填写
例如
...
6.在本脚本最下方代码if__name__=='__main__'：下配置UA变量
用户代理参数可以抓包任意接口在页眉中看到
定时运行每半个小时一次
'''#行：59
进口请求#行：60
进口再#行：61
...
进口Hashlib#行：66
进口时间#行：67
从……起urllib.parse进口urlparse，parse_qs#行：68
checkDict={'onenotischeck':['第一篇文章','过检测'],}#行：71
push_num=[1,2 ]#行：72
定义 getMsg ():#行：73
OOOOOOOO0OOOOOO0OOOO0OO0O0='v1.5f'#行：74
OOOOOOOOOO000000000OO0=''#行：75
    尝试:#行：76
OOOO00O0O00OO000O0='http://175.24.153.42:8881/getmsg'#行：77
O00OO0OO0OO0OOOO0OO0O={'类型':'zhyd'}#行：78
OOOOOOOOOO000000000OO0=请求。得到 (OOOO00O0OO00OO000O0，params=OOOOO0OO0OOOO0OO0OO0，timeout=2 )#行：79
OO0O000OO0OO0OO0OOO=OOOOOOOO000O000OO0。JSON ()#行：80
O00O00OOOO0000OOO=OOOO000OOOOOOOOOO。得到 ('版本')#行：81
OO00OOOOOOOOOOOOOOOOOOOOOO=OOOO000OOOOOOOOOOOO。得到 ('gdict')#行：82
OOOOOO00OOOO00OOOOOOOO=OOOO000OOOOOOOOOO。得到 ('gmmsg')#行：83
        打印 ('系统公告:'，OOOOOO00OOOO00OOOOOOOO)#行：84
        打印 (F'最新版本{O00O00OOOO0000OOO},当前版本{OOOOOOOO0OOOOOO0OO0OO0}')#行：85
        打印 (F'系统的公众号码字典{Len(OO00O0OO0OO0OO0OO0OO)}个:{OO00O0OO0OO0OO0OO0OO}')#行：86
        打印 (F'本脚本公众号码字典{Len(checkDict。值())}个:{列表(checkDict。键())}')#行：87
        打印 ('='*50 )#行：88
    除……之外例外作为O0OOOO000OOOOOO0O0：#行：89
        打印 (O0OOOO000OOOOOO0O0)#行：90
        打印 ('公告服务器异常')#line:91
定义 推 (OO0O000OOOOOOOOOO0O，OOOOOO000OOOO00OOOO，OOOO0000OOOO00OOOO00，OOOOOOOO0000OOOOOO00，OOOOOOOOOOOOOOOOOO，OOOOOOOOOOOOOOOO00OOOOOOOOOO):#行：92
OO00OO0OO0OO0OO0OO0O0='''
<body onload="window.location.href='LINK'">
<p>文本</p><br>
<p><a href="http://175.24.153.42:8882/lookstatus?key=KEY&type=TYPE">查看状态</a></p><br>
</body>
'''#行：98
OO0OOOO0OO00OO00OO=OO00OO0OO0OO0OO0OO0OO0OO0。取代 ('TITLE'，OOOO000OOOOOOOOOOOOOO).取代 ('链接'，OOOO0000000000000).取代 ('文本'，OOOO000000O0OO00).取代 ('类型'，OOOOOO000000OOOO00).取代 ('键'，OO0O0OO00OOOOOOOO0)#行：100
O0OO0OO0OO00000O={"AppToken"：AppToken，"内容"：OO0OOOO0OO00OO00OO，"摘要"：OO00000OOOOOOOOOOOOOO，"contentType":2,"UID":[O0O0OOOO0OOOOOO0OOOOO]}#行：107
O0O000O000OO0OO0O='http://wxpusher.zjiecode.com/api/send/message'#行：108
    尝试:#行：109
OOO000OOOOOOOO0OOOO=请求。邮件 (URL=OO000O000OO0OO0，json=OOOO0OO0OO00000O).文本 #行：110
        打印 ('推送结果：'，OOO000OOOOOOOO0OOOOO)#行：111
        返回 正确 #行：112
    除……之外例外作为OO00O0OOO0OOOO0000O：#行：113
        打印 ('推送失败！')#行：114
        打印 ('推送结果：'，OO00OOOOOO00OOOO0000O)#行：115
        返回 假的 #行：116
定义 getinfo (OOOOOOOO00OO00OO00000):#行：117
    尝试:#行：118
O00O0O00OO0OO0OO0O0=请求。得到 (OOOOOOOO00OO00OO00000)#行：119
